...
export default function SubmissionDetail({ submission, onBack }: SubmissionDetailProps) {
  ...
  if (hasAdvancedGrading) {
    return (
      <div className="space-y-6">
        ...
        <div className="glass-card-elevated p-6">
          <h3 className="font-display font-semibold text-foreground mb-2">JD Alignment</h3>
          <p className="text-foreground">{submission.jdAlignment}%</p>
          <p className="text-sm text-muted-foreground mt-2">{submission.documentAnalysis?.competitivePositioning}</p>
        </div>
  ...
  return <div className="text-center py-12"><p className="text-muted-foreground">Loading submission details...</p></div>;
}
